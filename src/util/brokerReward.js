...
class BrokerReward {
    constructor(topicsArray) {
        ...
        logger.debug({ event: 'Event Stream constructor called', functionName: 'constructor in BrokerReward', features: Kafka.features, brokers, topicsArray });
    ...
    _ConnectConsumer(topicsArray) {
        try {
            ...
            const consumer = new Kafka.KafkaConsumer({
                ...
                "client.id": config.kafkaBroker.consumerConfig.client_id + (POD_NAME != '' ? '-'+POD_NAME : ''),
            ...
            consumer.on('event.error', function(err) {
                logger.error({
                    event: 'Error thrown',
                    functionName: "_ConnectConsumer in BrokerReward",
                    error: err
                });
            });
    ...
    produceMessage(msg, topicName) {
        ...
        logger.debug({ event: " About to produce message", function: "produceMessage in BrokerReward" });
