...
const accountStatementTemplateMerchant = accountData => {
    ...
    console.log("ACCOUNTDATA DATA +++++++++++++++++++++++", JSON.stringify(accountData.data))
    console.log("ACCOUNTDATA PAYLOAD ********************", JSON.stringify(accountData.payload))
    console.log("accountData.data.length - 2", accountData.data.length - 2)
    ...
    try {
        ...
        if (accountData.data.length === 0) {
            ...
            accountData.data.forEach((number) => {
                console.log("number[number.length - 3] / 100 ================",number[number.length - 3] / 100)
            ...
            console.log("FAHAD TOTAL CREDIT <> ========================================", totalCredit)
            console.log("FAHAD TOTAL DEBIT <> ========================================", totalDebit)
		...
		<div class="statementSummary">
			...
			<div class="statementDetails">
                ...
                <div>Total Credit Transactions: <b>${totalFee ? totalFee.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") : 0}</b></div>
