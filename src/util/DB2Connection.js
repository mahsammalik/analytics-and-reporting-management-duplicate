...
class DatabaseConn {
    ...
    async insertTransactionHistory(schemaName, tableName, data) {
        ...
        if(tableName === config.reportingDBTables.DONATION)
        {
            try {
                let conn = await open(cn);
                const stmt = conn.prepareSync(`INSERT INTO ${schemaName}.DONATION (AMOUNT, CHANNEL, "DATE", EMAIL, FAIL_REASON, FUND, MSISDN, ORGANIZATION, STATUS, TRANS_ID) VALUES(${data.amount}, '${data.channel}', TIMESTAMP_FORMAT('${data.transactionTime}','YYYY-MM-DD HH24:MI:SS'), '${data.email}', '${data.failureReason}', '${data.fund}', ${data.msisdn}, '${data.organization}', '${data.transactionStatus}',${data.TID});`);
                stmt.executeSync();
                stmt.closeSync();
                conn.close(function(err) {});
                console.log("insert done");
        ...
        if(tableName === config.reportingDBTables.BUS_TICKET)
        {
            try {
                let conn = await open(cn);
                const stmt = conn.prepareSync(`INSERT INTO ${schemaName}.BUS_TICKET (AMOUNT, BOOKING_DATE, BOOKING_ID, CHANNEL, CNIC, DESTINATION, DISCOUNT, EMAIL, FEE, GENDER, MSISDN, ORIGIN, ORIG_PRICE, PRICE, PROMO, SEATS, SEAT_NUMBER, SERVICE, STATUS, STATUS_REASON, TRANS_ID, TRAVEL_DATE) 
        ...
        if(tableName === config.reportingDBTables.EVENT_TICKET)
        {
            try {
                let conn = await open(cn);
                const stmt = conn.prepareSync(`INSERT INTO ${schemaName}.EVENT_TICKET (AMOUNT, BOOKING_ID, BOOK_DATE, CHANNEL, CITY, CNIC, DISCOUNT, EMAIL, EVENT, EVENT_DATE, FAIL_REASON, MSISDN, NUMBER_OF_SEATS, PARTNER, PRICE, PROMO_AMOUNT, PROMO_APPLIED, REVENUE, SEAT_CLASS, SUCCESSFUL, TRANS_ID) 
